...
export function CustomTemplatePreview({ templateId }: { templateId: string }) {
  ...
  return <main><header className="preview-bar"><a href="/templates">← 返回模板中心</a><span>自定义模板预览 · Mock 数据</span><a className="studio-link-button" href={`/template-studio/${template.id}/edit`}>编辑模板</a></header><StudioDocument document={template.document} /></main>;
}
