...
import { bestTrackBrandedExtension, BrandedRuntimeProvider, type TrackingPageQuery } from "@standhigher/besttrack-page-extension";
...
export function BrandedDemo({ getSessionToken }: { getSessionToken?: GetSessionToken }) {
  ...
  const query = useCallback<TrackingPageQuery>(async (request) => {
    ...
    if (!getSessionToken) return {
      trackingNumber, status: "In transit", carrier: "BestTrack demo carrier", latestEvent: "Shipment accepted at the regional hub", destination: "Shanghai",
      shipments: [
        {
          ...
          recommendations: [{ id: "shipping-protection", title: "Shipping protection", description: "Extra assurance for your next delivery.", imageUrl: "https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?auto=format&fit=crop&w=640&q=80", href: "https://example.com/products/shipping-protection", price: { amount: 1200, currencyCode: "USD" } }]
        },
        {
          ...
          recommendations: [{ id: "delivery-alerts", title: "Delivery alerts", description: "Receive updates at every milestone.", imageUrl: "https://images.unsplash.com/photo-1556740749-887f6717d7e4?auto=format&fit=crop&w=640&q=80", href: "https://example.com/products/delivery-alerts", price: { amount: 800, currencyCode: "USD" } }]
        },
  ...
  return <BrandedRuntimeProvider query={query}><Page fullWidth><BlockStack gap="300">
