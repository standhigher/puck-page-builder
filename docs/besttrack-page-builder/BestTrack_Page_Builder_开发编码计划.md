...
- Admin UI 从 V0.1 起统一遵守 Shopify Polaris，消费者页面保留品牌化能力。

## 2. 版本总览

| 版本 | 目标 | 人工验收重点 | 状态 |
|---|---|---|---|
| V0.1 | 基础工程与 Polaris EditorShell | 工程可运行、编辑器布局和 Polaris 风格 | 待开发 |
...
### V0.1 基础工程与 Polaris EditorShell
...
- 建立 `core`、`editor`、`adapters`、`runtime`、`renderer`、`ui/polaris` 目录；
- 接入 Puck 和 Polaris UI 适配层；
...
- 可选择真实商品、集合、图片和菜单；
- 模板切换不会产生非法或丢失数据；
- Tracking Page 可按商家品牌定制，不被 Polaris 风格覆盖；
- 非法资源和失效资源有明确降级提示。
...
当前只启动 **V0.1 基础工程与 Polaris EditorShell**。V0.1 人工验收通过前，不开发 PageDocument、Extension API 和其他后续能力。
