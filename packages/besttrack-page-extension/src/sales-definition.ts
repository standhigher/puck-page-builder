...
function validateCategories(props: Record<string, unknown>) {
  ...
  if (typeof props.collectionId !== "string" || !/^gid:\/\/shopify\/Collection\/\d+$/.test(props.collectionId)) issues.push({ path: "props.collectionId", message: "Use a Shopify collection GID selected by an authorized integration." });
  return issues;
}
...
const salesBlocks: BlockDefinition[] = withTemplatePolicy([
  ...
  { type: "besttrack.sales.product-categories", version: 2, label: "Product categories", category: "BestTrack Sales", targets: ["web"], defaultProps: { heading: "Shop by category", collectionId: "gid://shopify/Collection/1", collectionLabel: "Featured collection" }, defaultVariant: "hero", variants: [{ id: "hero", label: "Sales Hero" }, { id: "commerce", label: "Commerce" }, { id: "grid", label: "Grid", theme: { "color.surface": "#f8fafc" } }], fields: { heading: shortText("Heading"), collectionId: text("Collection ID", { group: "Resource", description: "Stable Shopify collection GID selected by an authorized integration." }), collectionLabel: shortText("Collection label") }, validate: validateCategories, render: { web: SalesProductCategoriesBlock, editor: SalesProductCategoriesEditor } },
...
/** Sales v2 uses stable IDs within newly created development documents. */
export function createSalesTemplate(): TemplateDefinition {
  return { id: "besttrack.sales", version: 2, name: "Sales", target: "web", source: "built-in", requiredBlocks: salesBlocks.map((block) => block.type), theme: { "color.primary": "#000000", "color.background": "#f7f5f0", "color.surface": "#ffffff", "color.text": "#0a0a0a", "color.muted": "#6b6b6b", "color.border": "#dfddd7", "font.family": "Arial, Helvetica, sans-serif", radius: "10px", spacing: "24px" }, create: () => createPageDocument({ pageId: "besttrack-sales", target: "web", templateId: "besttrack.sales", templateVersion: 2, settings: { locale: "en", seoTitle: "Shop and track your order" }, blocks: salesBlocks.map((block, index) => ({ id: "sales-" + (index + 1), type: block.type, version: block.version, props: block.defaultProps as Record<string, JsonValue>, variant: block.defaultVariant, style: {} })) }) };
}

export const bestTrackSalesExtension: PageBuilderExtension = Object.freeze({ name: "besttrack.sales", version: "0.8.0", fields: [{ type: "besttrack.sales.text", component: SalesTextField }], blocks: salesBlocks, templates: [createSalesTemplate()] });
